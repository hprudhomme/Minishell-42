...
void    set_up_output(t_mem *mem, t_cmdlst *lst, int i)
{
    if (lst->next == NULL)
    {
        if(lst->write_in)
        {
            mem->exec_loop->redirect_path = find_path_redirect_file(getenv("PWD"), lst->write_in[i], mem->exec_loop->redirect_path);
            mem->exec_loop->fdout = open( mem->exec_loop->redirect_path, O_CREAT | O_WRONLY | O_TRUNC, S_IRWXO | S_IRWXU | S_IRWXG);
        }
        else if (lst->append_in)
        {
            mem->exec_loop->redirect_path = find_path_redirect_file(getenv("PWD"), lst->append_in[i], mem->exec_loop->redirect_path);
...
void execute3(t_cmdlst *lst, char **path_tab, char **env, t_mem *mem)
{
    ...
    nb_outfiles = 0;
    if (lst->write_in)
        nb_outfiles += tab_2d_len(lst->write_in);
    if (lst->append_in)
        nb_outfiles += tab_2d_len(lst->append_in);
    while (lst)
    {   
        if (nb_outfiles > 0)
            i = 0;
